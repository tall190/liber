...
<strong>Recall(재현율)</strong>: "실제로 관련 있는 문서 중, 몇 개를 찾아냈는가?"
- Recall 1.0 = 완벽 (Brute Force만 가능)
- Recall 0.95 이상 = 실용적 수준
- Recall 0.80 이하 = 체감되는 품질 저하

<strong>Latency</strong>: 질문 넣고 결과 나올 때까지 걸리는 시간이에요. 실서비스에서는 p99가 중요해요. p50이 5ms라도 p99가 500ms면 일부 사용자는 매우 느린 경험을 하거든요.
...
대표적인 벤치마크 기준으로 1,536차원 벡터에서 ef=128 언저리가 Recall 0.95~0.96, Latency 15~20ms 구간에 해당해요. 데이터셋과 하드웨어에 따라 크게 달라지므로 직접 측정이 필요해요.
