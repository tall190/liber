...
RDB는 값이 <strong>정확히 일치</strong>하거나 <strong>포함</strong>되는지만 판단할 수 있거든요. 의미가 비슷한지는 판단 못 해요.
...
<strong>임베딩(Embedding)</strong>은 텍스트를 수백~수천 차원의 숫자 배열(벡터)로 변환하는 기술이에요. 핵심은 이거예요 — 의미가 비슷한 텍스트는 벡터 공간에서 <strong>가까운 위치</strong>에 놓여요.
...
*동일한 임베딩 모델로 문서와 쿼리를 변환해야 정확한 유사도 비교가 가능해요.*
...
*벡터 DB 검색 품질(Recall)이 RAG 전체 품질을 결정해요. 관련 문서를 못 찾으면 LLM이 아무리 좋아도 정확한 답을 낼 수 없어요.*

<strong>① 사전 준비 (Indexing)</strong> — 문서를 벡터 DB에 저장해요.
...
<strong>② 검색 (Retrieval)</strong> — 질문을 벡터로 만들고 가장 가까운 청크들을 찾아요.

<strong>③ 생성 (Generation)</strong> — 검색된 청크들 + 질문을 LLM에 넘기면 문서 기반 답변이 나와요.

<strong>핵심은 이거예요.</strong> 벡터 DB 검색 품질이 RAG 전체 품질을 결정해요. 관련 문서를 못 찾으면 LLM이 아무리 좋아도 정확한 답을 낼 수 없거든요.
...
*텍스트 RAG엔 코사인 유사도가 기본값이에요. 정규화된 벡터라면 내적(Dot Product)과 동일한 결과가 나와요.*

<strong>코사인 유사도</strong> — 두 벡터 사이의 각도를 기준으로 유사도를 측정해요. 벡터 크기는 무시하고 방향만 비교하죠. 텍스트 유사도 검색의 기본값이에요.

<strong>내적 (Dot Product)</strong> — 방향과 크기 모두 반영해요. 벡터가 정규화되어 있으면 코사인 유사도와 동일한 결과가 나와요.

<strong>유클리드 거리 (L2)</strong> — 두 점 사이의 직선 거리예요. 벡터가 정규화되어 있으면 L2와 코사인 유사도는 수학적으로 동일한 결과를 내요. 정규화되지 않은 상태에서는 이미지·수치 데이터 검색에 더 자주 쓰여요.
...
*ef 값을 높일수록 Recall은 올라가지만 Latency도 증가해요. 대부분의 RAG 서비스는 ef=128~256 구간이 적정점이에요.*

<strong>Recall(재현율)</strong>: "실제로 관련 있는 문서 중, 몇 개를 찾아냈는가?" Recall@10이란 상위 10개 결과에 실제 관련 문서가 포함된 비율이에요.

| Recall 수준 | 의미 |
|------------|------|
| 1.0 (100%) | 모든 관련 문서를 찾음. FLAT(Brute Force)만 수학적으로 보장 가능 |
| 0.95~0.98 | <strong>대부분의 RAG 서비스 적정 목표</strong>. 사용자가 품질 저하를 체감하지 않는 수준 |
| 0.99+ | 금융·의료·법률 등 정확도가 절대적인 도메인에서 요구. Latency 비용이 큼 |
| 0.80 이하 | 체감되는 품질 저하. 관련 문서 10개 중 2개 이상을 놓침 |

<strong>Latency</strong>: 질문 넣고 결과 나올 때까지 걸리는 시간이에요. 실서비스에서는 평균이 아니라 꼬리 분포가 중요해요.

| 지표 | 의미 |
|------|------|
| p50 (중앙값) | 절반의 요청이 이 시간 안에 응답 |
| p95 | 95% 요청이 이 시간 안에 응답 |
| p99 | <strong>실서비스 핵심 지표</strong>. p50이 5ms라도 p99가 500ms면 100명 중 1명은 매우 느린 경험을 함 |

가장 중요한 원칙은 이거예요.

> 더 정확하게 찾을수록 더 오래 걸리고, 더 빠를수록 덜 정확해져요.

많은 RAG 서비스의 합리적인 목표 구간은 <strong>Recall 0.95~0.98, p99 Latency 50ms 이하</strong>예요. 정확도가 더 중요한 금융·의료 같은 도메인에서는 Recall 0.99+를 요구하기도 해요. 이 목표를 현실적으로 달성하려면 어떤 알고리즘을 써야 할까요.

## ANN 알고리즘: 왜 정확도를 조금 포기하는가

수억 개 벡터를 모두 비교하는 건 현실적으로 불가능해요. 그래서 대부분의 벡터 DB는 <strong>근사 최근접 이웃(ANN)</strong> 알고리즘을 사용해요.

### FLAT — Brute Force

![FLAT 알고리즘](../../assets/vector-db-basics_flat_image.png)
*모든 벡터와 비교하므로 Recall 100%를 보장해요. 데이터가 많아질수록 비례해서 느려지는 게 단점이에요.*
...
*전체를 탐색하지 않고 가까운 클러스터 일부만 탐색해요. nprobe 값으로 탐색 범위와 속도를 조절할 수 있어요.*
...
*현재 가장 널리 쓰이는 ANN 알고리즘이에요. Weaviate, Qdrant, pgvector 등이 기본값으로 사용해요.*
...
속도와 정확도 균형이 가장 좋고, 실시간 삽입도 가능해요. Weaviate, Qdrant, pgvector가 HNSW를 기본으로 사용해요.
...
`ef` 파라미터가 Recall-Latency 균형을 직접 제어해요. 아래는 대표적인 벤치마크 경향을 보여주는 예시 수치예요. 실제 수치는 하드웨어·데이터셋·차원에 따라 크게 달라지므로 직접 측정이 필요해요.

| ef 값 | Recall | Latency | 해석 |
|-------|--------|---------|------|
| 32 | ~88% | ~4ms | 빠르지만 관련 문서 10개 중 1개 이상 놓침 |
| 64 | ~93% | ~9ms | 속도 우선 서비스에서 최소 기준선 |
| 128 | ~96% | ~18ms | <strong>대부분의 RAG 서비스 권장 시작점</strong> |
| 256 | ~98% | ~35ms | 높은 정확도 필요 시. p99 50ms 이내 가능 |
| 512 | ~99.3% | ~75ms | 금융·의료 등 Recall 최우선 도메인 |
| FLAT | 100% | ~500ms | 완전 탐색. 프로덕션 실시간 서비스 부적합 |

도메인에 따라 어디를 노려야 하는지도 달라져요.

| 도메인 | Recall 목표 | p99 Latency | ef 범위 |
|--------|-----------|-------------|---------|
| 일반 사내 문서 RAG | 0.95~0.98 | 50ms 이하 | 128~256 |
| 고객 대면 서비스 | 0.97+ | 30ms 이하 | 256+ |
| 금융·의료·법률 | 0.99+ | 100ms 허용 | 512+ |
| 내부 도구·프로토타입 | 0.90+ | 100ms 허용 | 64~128 |
...
‡ RAM이 충분하면 수억 개까지 확장 가능하지만, 그 이상 규모에서는 비용 대비 DiskANN이 유리해요.
