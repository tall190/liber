...
AI는 코드를 읽기 시작해요. 컴포넌트 트리를 파악하고, 클릭 핸들러를 추적하고, 무슨 일이 벌어지는지 추론하려 합니다. 그런데 버튼에 이름이 없어요.
...
AI 입장에서 이 코드는 모호해요. `handleClick`은 다른 버튼들도 쓰고 있고, `btn-primary`는 스타일 정보일 뿐이고, 텍스트 "저장"은 다음 릴리스에 "저장하기"로 바뀔 수도 있죠. AI는 이 버튼이 정확히 어떤 역할을 하는지 확신하기 어렵습니다.
...
`data-testid="save-draft-button"` — 이 속성 하나가 AI에게 명확한 계약을 만들어줍니다. "이 버튼은 임시저장 버튼이에요. 텍스트가 바뀌어도, 스타일이 바뀌어도, 이 이름으로 찾아요."
...
새 기능을 만들 때 AI에게 맥락을 설명하는 시간이 줄어들어요. "이 폼은 광고 소재 등록 폼이고, 제출 버튼은 `submit-creative-button`이야" — 코드 자체가 이미 그 설명을 담고 있거든요. AI는 컴포넌트 트리를 읽고 의도를 파악합니다.
...
AI는 이 구조를 보고 "광고 소재 업로드 페이지에, 소재 등록 폼이 있고, 제목 입력과 URL 입력, 제출 버튼이 있다"는 맥락을 정확하게 파악합니다. 개발자가 따로 설명하지 않아도요.
...
다국어 지원을 처음부터 고려한다면, `data-testid` 도입은 선택이 아니라 필수예요. i18n 라이브러리로 텍스트를 추상화한 것처럼, testid로 UI 식별도 추상화해야 해요.
...
정답은 **데이터의 고유 식별자를 함께 쓰는 것**이에요. 인덱스(0, 1, 2...)는 정렬이나 필터링을 적용하면 바뀌지만, 데이터 ID는 변하지 않아요.
...
정렬 → 특정 항목 선택 → 수정 → 저장 같은 시나리오를 AI가 처음부터 끝까지 수행할 수 있어요.

---

## 개발이 빨라지면, QA가 병목이 된다

AI 코딩 어시스턴트 덕분에 개발 속도가 올라갔어요. 하루에 만들 수 있는 기능의 수가 늘었어요. 그러면 다음 질문이 생기죠.

**검증 속도도 함께 올라갔나요?**

개발자가 10개 기능을 만들어도, QA가 3개밖에 검증 못하면 전체 속도는 3개로 수렴해요. 개발 속도가 빨라질수록, 병목은 QA로 이동합니다.

해결책은 QA도 AI와 함께 하는 거예요. E2E 테스트를 AI가 작성하고, 회귀 케이스를 AI가 추적하고, 릴리스 전 검증을 AI가 실행하고.
...
디자인이 바뀌어도, 텍스트가 바뀌어도, 언어가 바뀌어도 — 이 테스트는 흔들리지 않아요.

개발 단계에서 붙인 `data-testid`가 QA 단계의 자동화를 가능하게 합니다. 같은 이름이 두 단계를 연결하는 거예요.
...
[ProofSource.ai의 연구](https://proofsource.ai/2026/01/agent-browser-the-accessibility-first-approach-to-browser-automation/)에 따르면, AI 에이전트가 전체 DOM 대신 접근성 트리(Accessibility Tree)만 읽도록 했을 때 컨텍스트 토큰이 **93% 절감**됐어요. AI는 의미 단위로 요약된 구조를 훨씬 잘 이해해요.

스크린리더를 위해 작성한 ARIA 속성이 AI 에이전트에게도 정확한 인터페이스가 되는 거예요. [BrowserStack의 Playwright MCP 분석](https://www.browserstack.com/guide/modern-test-automation-with-ai-and-playwright)도 같은 결론이에요 — ARIA 속성이 AI 테스트 생성 정확도에 직접 영향을 줍니다.
...
"알겠어요, 근데 기존 코드베이스가 수백 개 컴포넌트인데 어떻게 하나요?" — 당연한 걱정이에요.

다행히 AI 도구 자체가 이 작업을 도와줄 수 있어요.
...
명확한 `data-testid`는 사람인 개발자가 테스트를 쓸 때도 편해요. 시맨틱 HTML은 스크린리더 접근성을 높이고, 다국어 서비스에서 텍스트와 식별자를 분리해줘요. 명확한 컴포넌트 경계는 코드 리뷰를 쉽게 만들어요.
...
*다음 글: [AI-ready Marketing — 당신의 브랜드를 AI에게 설명할 준비가 되었나요?](/blog/geo-for-brands) — 같은 원리의 다른 이야기. 이번엔 개발팀이 아닌 브랜드가 주인공이에요.*
